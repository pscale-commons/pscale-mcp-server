...
**Supabase branches blocked** (18 April 2026): attempted to create a `dev` branch for isolation; it went `MIGRATIONS_FAILED` because the first migration `add_paid_and_saved_games` (version 20260402) tries `ALTER TABLE public.users ADD COLUMN IF NOT EXISTS paid boolean DEFAULT false` — but `public.users` doesn't exist on a fresh branch (it's a legacy Onen table that was never added to the migration chain). Main has the table from long ago, branches don't. Same failure killed the earlier `fresh-build-db` branch. Fix is in the upstream migrations repo (probably xstream-play) — make the first migration tolerate a missing `users` table via `IF EXISTS` guarding, e.g. wrap in a `DO $$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.tables WHERE ...) THEN ... END IF; END $$`. Until that lands, Supabase branch isolation is not available. For now: use naming-prefix convention for smoke tests (`test-...`, `smoke-...`) or just accept writes go to production and be careful.
